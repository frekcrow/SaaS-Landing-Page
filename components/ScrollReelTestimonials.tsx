...
import { Stethoscope, Heartbeat, FirstAid, Pill, Syringe, Flask, Virus, Thermometer } from "@phosphor-icons/react";
...
const QUOTE_CLASSES =
  "m-0 text-lg font-medium leading-[1.3] tracking-[-0.02em] text-foreground sm:text-[22px] max-md:font-pinglight max-md:font-light";
...
function Cell({ index = 0 }: { index?: number }) {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-700 blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]"
      style={{ width: CELL, height: CELL }}
    />
  );
}

/* Featured portrait tile without the neon sheen and desaturation */
...
/* Per-word split. For Arabic, splitting by character breaks cursive ligatures. 
 * By splitting by word, the text stays properly connected and we just stagger 
 * each word instead of each letter. */
function Words({
  text,
  startIndex,
  staggerMs,
  ...
  return (
    <>
      {words.map((word, wi) => {
        ...
        idx++; // increment per word
        return (
          <React.Fragment key={wi}>
            <span
              className="scroll-reel-char whitespace-nowrap"
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-border bg-white shadow-[inset_0_2px_0_rgba(255,255,255,1)] outline-none focus-visible:ring-2 focus-visible:ring-ring md:min-h-[320px] md:flex-row",
        "dark:bg-muted dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        className
      )}
    >
      {/* Reel section */}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-slate-300 dark:text-muted-foreground/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
            ...
            >
              <p className={QUOTE_CLASSES}>
                <Words
                  text={current.quote}
                  startIndex={0}
                  staggerMs={charStaggerMs * 4} // Increase delay slightly per word since there are fewer words than characters
                />
              </p>
              <p className={AUTHOR_CLASSES}>
                <Words
                  text={current.author}
                  startIndex={current.quote.split(" ").length + 2}
                  staggerMs={charStaggerMs * 4}
                />
              </p>
            </div>
          </div>
        ...
        {/* Controls */}
        <div className="mt-6 flex items-center gap-1.5 md:mt-0" dir="ltr">
          <button
            type="button"
            onClick={() => paginate(-1)}
            disabled={index === 0}
            aria-label="Previous testimonial"
            className="grid h-8 w-8 cursor-pointer place-items-center rounded-full border border-foreground/15 bg-transparent p-0 text-foreground transition-[opacity,transform] duration-200 hover:bg-slate-50 disabled:cursor-default disabled:opacity-40"
          >
            <svg className="h-4 w-4 opacity-70" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
